...
In this project, I responsed for cost caluation in Task 1 and comparsion of path planning algorithms in Task A3. Genai, like: chatgpt, gork and deepseek, took a somewhat impreative role in my jobs to accelerate every stages. 
In Task 1, AI instantly translated complex cost-function equations into clean, vectorized Python code and provide helps in debugging syntax and indexing errors. Finding the optimium cost of different scienarios withour manual calculations. Task A3 required implementing and comparing multiple algorithms on the same grid. Here, I fed it my working A* implementation and asked it to convert the core search loop into D* and Theta* with rewriting, and even a Genetic Algorithm, while keeping my exact Node class, cost function, and visiable map intact. Each conversion took seconds to generate and minutes to verify—letting me test extra 2 algorithms. Ultimately, AI wasn’t a magic button; it was the fastest, most patient pair-programmer I’ve ever had. It turns weeks of tedious coding into days of focused refinement and exploration. Honestly, working with AI felt like having a brilliant, tireless teammate who never sleeps and iterates at light speed. It turned frustration into flow and transformed a good project into an outstanding one. I’m genuinely grateful—AI has become an indispensable partner in my learning and projects.
